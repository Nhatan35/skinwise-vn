...
- `GET /api/admin/products` and `PATCH /api/admin/products/[id]/verification-status` require authentication plus `AppUserProfile.role = "ADMIN"`. The admin authorization helper reads the role from the SkinWise app profile by current Auth.js user id; it does not trust client input or add role claims to the Auth.js session.
- The admin product review foundation can list all product verification states and update only `verificationStatus`. It does not hard-delete products, does not add `isActive`, and does not expose raw MongoDB internals, `source`, `createdByUserId`, OAuth data, tokens, secrets, or raw sessions.
