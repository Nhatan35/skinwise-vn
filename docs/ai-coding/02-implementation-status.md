...
Routine Analysis API exists with deterministic fallback metadata only and now rate-limits authenticated analyze requests per user. It has an explicit provider-to-product mapper for future provider-backed routine analysis results, but it is not wired to call the AI Provider Abstraction yet and does not call OpenAI, Gemini, LLM clients, external APIs, Product/Ingredient explanation modules, dashboard, Journal, or RoutineLog UI features.
...
```txt
TASK AI-005 - Wire Validated AI Provider into Routine Analysis Use Case with Safe Fallback. Do not begin Ingredient Explanation, Product submission, admin product management, Journal, skin score, image upload, or medical diagnosis without a new task.
```
