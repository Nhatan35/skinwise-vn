...
| Product mini database | Done | `GET /api/products`; `GET /api/products/[id]` | `/products` catalogue UI, `/products/[id]` detail UI, Routine Builder Product Picker, and SkinJournal product selection/name resolution | `products` collection helpers and canonical indexes | Unit/API/index/client/source checks | Read-only authenticated foundation; returns only reviewed/verified products; list UI parses `data.items`; detail UI parses `data.product`; `/products` supports search/filtering and ProductCard detail navigation; no Product CRUD, POST API, product submission, includeMine UI, admin, saved product library, seed script, external APIs, AI recommendation, skin score, routine integration, or image upload |
| Product Detail UI / PRODUCT-UI-002 | Done | `GET /api/products/[id]` | Protected `/products/[id]` detail page | No new DB | Unit/source checks | ProductDetail has loading, error, not-found, retry, educational disclaimer, Back to products, and public Product DTO field display only |
