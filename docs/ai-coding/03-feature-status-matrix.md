...
| Routine Logs | Completed | `GET /api/routine-logs`; `PUT /api/routine-logs`; `DELETE /api/routine-logs/:id` | Daily controls inside `/routines`; dedicated `/routine-logs/today` checklist page with update and delete support | `routine_logs` with unique date/routine index | Unit/API/client/source checks; production smoke | Today routine log update, routine log deletion support, dedicated Today Checklist access, and dashboard reflection are covered without adding analytics, streaks, or new collections |
...
| E2E Tests | Partially completed | Test-only Auth.js Credentials provider enabled only for `APP_ENV="test"` and `E2E_TEST_AUTH="true"` | Playwright smoke specs exist | Safe local/test MongoDB URI plus deterministic product and ingredient seed command for E2E | Playwright smoke/authenticated specs pass locally when local MongoDB is available | Public landing page, unauthenticated protected-route redirects, authenticated dashboard access, Skin Profile create/update, Product Catalogue, Product Detail, Saved Products save/list/remove, and Ingredient Library search/detail/explanation are covered; Routine, Journal, full Dashboard summary, and real Google OAuth login are not tested in CI |
