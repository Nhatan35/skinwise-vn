...
| E2E Tests | Partially completed | Test-only Auth.js Credentials provider enabled only for `APP_ENV="test"` and `E2E_TEST_AUTH="true"` | Playwright smoke specs exist | Safe local/test MongoDB URI plus deterministic product and ingredient seed command for E2E | Playwright smoke/authenticated specs pass locally | Public landing page, unauthenticated protected-route redirects, authenticated dashboard access, Skin Profile create/update, Product Catalogue, Product Detail, and Ingredient Library search/detail/explanation are covered; Routine, Journal, full Dashboard summary, and real Google OAuth login are not tested in CI |
