...
| SkinJournal | Done | `POST /api/skin-journal`; `GET /api/skin-journal`; `PATCH /api/skin-journal/[id]`; `DELETE /api/skin-journal/[id]` | `/journal` timeline UI with list/create/edit/delete | `skin_journals` collection helper and canonical indexes | Unit/API/index/source checks | Authenticated, user-scoped, one entry per localDate, duplicate creates return `CONFLICT`; future image fields remain excluded; no image upload, AI journal analysis, calendar heatmap, analytics view, or product name resolution |
