...
| RoutineLog | Done | `GET /api/routine-logs?localDate=YYYY-MM-DD`; `PUT /api/routine-logs` | No UI in RL-001 | `routine_logs` collection helper + unique/query indexes | Unit/API/repository/use-case checks | Backend foundation only; canonical PUT upsert by `userId + routineId + localDate`; validates routine ownership and completedStepIds; RL-002 will add UI |
