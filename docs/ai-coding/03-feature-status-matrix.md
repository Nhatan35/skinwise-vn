...
| Routine Logs | Completed | `GET /api/routine-logs`; `PUT /api/routine-logs` | Daily controls inside `/routines`; dedicated `/routine-logs/today` checklist page | `routine_logs` with unique date/routine index | Unit/API/client/source checks; production smoke | Routine log status update, dedicated Today Checklist access, and dashboard reflection are covered without adding analytics, streaks, or new collections |
