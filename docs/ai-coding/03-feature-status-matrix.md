...
| Routine Analysis | Completed | `POST /api/routines/[id]/analyze`; `GET /api/routines/[id]/analyses` | Analysis panel inside `/routines` | `routine_analyses`; `rate_limits` | Unit/API/source checks; production smoke; authenticated E2E spec with targeted duplicate-key warning guard | Playwright covers authenticated routine analysis using the existing mock/deterministic provider path; duplicate suggestion titles render with unique React keys; no real OpenAI/Gemini key required |
...
| Dashboard | Completed | `GET /api/dashboard?localDate=YYYY-MM-DD` | `/dashboard` | Reuses user-scoped collections | Unit/API/source checks; production smoke; authenticated E2E spec | Dashboard closeout now includes real profile completion, saved product count, 7-day routine consistency, and 7-day journal trend summaries from authenticated-user data; no fake analytics or new collections added |
...
| E2E Tests | Completed for local MVP core journey coverage | Test-only Auth.js Credentials provider enabled only for `APP_ENV="test"` and `E2E_TEST_AUTH="true"` | Playwright smoke and authenticated core journey specs exist | Safe local/test MongoDB URI plus deterministic product/ingredient seed and E2E-user-owned data reset command | Latest recorded local validation passed with 24 Playwright tests; requires local MongoDB and the project Node/npm baseline for reruns | Core MVP journey has authenticated Playwright coverage for Routine Builder, Routine Analysis, Today Checklist, Routine Log delete, Skin Journal, Settings/Data Control, account deletion request, and Dashboard reflection. Routine Analysis duplicate React key warning was addressed in the UI render layer and guarded in the local spec. Real Google OAuth production login and external production verification remain separate |
