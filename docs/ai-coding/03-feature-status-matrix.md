...
| AI Provider Abstraction | Done | No public API | N/A | No | Unit | Server-only `src/infrastructure/ai`; `MockAIProvider`, `ValidatedAIProvider`, `getAIProvider()`, and safe Routine Analysis provider failure classification are implemented; OpenAI/Gemini not implemented; no external AI API calls; no AI key required |
