...
| Runtime baseline | Partially completed | N/A | Node 24.x / npm 11.x documented for local, CI, and deployment | N/A | Local Node 24 validation passed except E2E blocked by missing local MongoDB | `.nvmrc`, `package.json` engines, GitHub Actions, README, and deployment docs target Node 24.x / npm 11.x; full E2E validation still requires MongoDB at the safe local/test URI |
...
| E2E Tests | Partially completed | Test-only Auth.js Credentials provider enabled only for `APP_ENV="test"` and `E2E_TEST_AUTH="true"` | Playwright smoke specs exist | Safe local/test MongoDB URI plus deterministic product seed command for E2E | Playwright smoke tests | Public landing page, unauthenticated protected-route redirects, authenticated dashboard access, Skin Profile create/update, Product Catalogue, and Product Detail are covered; Routine, Journal, full Dashboard summary, and real Google OAuth login are not tested in CI |
