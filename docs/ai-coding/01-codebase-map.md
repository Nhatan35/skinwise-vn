...
Current package state: **Post E2E-001 smoke test cleanup and deployment re-verification preparation**.
...
Current unimplemented areas are Product submission POST API, full admin product create/edit management, real OpenAI/Gemini provider integration, external LLM/API calls, SkinJournal saved-product embedding, dedicated SkinJournal AI analysis, skin score, image upload, marketplace/cart/payment, public sharing, ratings/reviews, and medical diagnosis. Lightweight admin product review API/UI for `verificationStatus` exists in v1.44/v1.45. Post-MVP v1.3 now includes a separate `/insights` calendar and self-tracked analytics view.
...
`src/app/api/products/route.ts` and `src/app/api/products/[id]/route.ts` are SkinWise-owned protected Product API read routes. They require `getCurrentUser()`, validate list query params with Zod, call Product use cases, and return Product DTOs without `_id`, raw ObjectId values, `createdByUserId`, or `source`. This foundation returns only `reviewed` or `verified` products and is consumed by the Routine Builder Product Picker, SkinJournal product selection/name resolution, `/products` Product Catalogue UI, `/products/[id]` Product Detail UI, and Saved Products product display. `src/app/api/admin/products/route.ts` and `src/app/api/admin/products/[id]/verification-status/route.ts` are ADMIN-only review routes for all-status listing and `verificationStatus` updates. They do not implement `POST /api/products`, `includeMine`, Product submission, full Product CRUD UI, marketplace/cart/payment, or image upload.

`src/app/api/saved-products/route.ts` and `src/app/api/saved-products/[productId]/route.ts` are SkinWise-owned protected Saved Products API routes. They derive `userId` from `getCurrentUser()`, validate `productId`, call saved-product use cases, return `SavedProductDto` without `userId`, and scope all list/save/remove behavior to the authenticated user. `POST /api/saved-products` confirms visible product existence and is idempotent for duplicates. `DELETE /api/saved-products/[productId]` removes only the saved-product record, never the product.
...
`src/app/(dashboard)/layout.tsx` protects the dashboard route group with `getCurrentUser()` and redirects unauthenticated users to `/api/auth/signin?callbackUrl=/dashboard`. `src/app/(dashboard)/dashboard/page.tsx` creates the real `/dashboard` URL and renders `DashboardOverview`; `/dashboard` is no longer placeholder-only. `src/app/(dashboard)/onboarding/skin-profile/page.tsx` creates the protected `/onboarding/skin-profile` URL and renders the Skin Profile onboarding form. `src/app/(dashboard)/products/page.tsx` creates the protected `/products` URL and renders the Product Catalogue UI for authenticated browsing/search/filtering of existing reviewed products. `src/app/(dashboard)/products/[id]/page.tsx` creates the protected `/products/[id]` URL and passes the route id into the Product Detail UI. `src/app/(dashboard)/saved-products/page.tsx` creates the protected `/saved-products` URL and renders the current user's saved-product list. `src/app/(dashboard)/routines/page.tsx` creates the protected `/routines` URL and renders the Routine Builder client component with Product Picker, Routine Analysis, and RoutineLog controls.
...
`auth.config.ts` is edge-safe and owns pure provider/config helpers. `src/auth.ts` owns the full server-side Auth.js setup with MongoDB Adapter gating. `src/proxy.ts` owns the Next.js 16 proxy wrapper for `/dashboard/:path*`, `/onboarding/:path*`, `/skin-profile/:path*`, `/routines/:path*`, `/journal/:path*`, `/products/:path*`, `/saved-products/:path*`, and `/ingredients/:path*`. `get-current-user.ts` maps Auth.js sessions to a minimal `CurrentUser` without querying `AppUserProfile`.
...
`product.schema.ts` owns strict list query validation for `GET /api/products` plus strict admin list/status schemas for the admin review API. `product.repository.ts` imports `server-only`, uses `getProductsCollection()`, applies default visibility for `reviewed` and `verified` products only, supports canonical search/filter params, supports ADMIN all-status search, and returns `null` for invalid ObjectId detail lookups without querying. `product.mapper.ts` converts `_id` to `id`, Dates to ISO strings, copies arrays, and omits `createdByUserId`, `source`, and raw ObjectId values. `product.client.ts` is client-safe and fetches `GET /api/products` with supported search/filter params and default `limit=50`, parsing visible products from `data.items` for UI consumers; it also fetches `GET /api/products/[id]` through `getProduct()` and parses detail responses from `data.product`. `admin-product.client.ts` is client-safe and calls the ADMIN review routes for all-status list and `verificationStatus` update. `components/product-catalogue.tsx` is the client-side `/products` catalogue surface with search, category, price range, skin type, and concern filters; loading/error/empty states; and Save/Saved state from the Saved Products client. `components/product-card.tsx` displays public Product DTO fields, links to product detail pages, and renders the saved-product toggle. `components/product-detail.tsx` is the client-side `/products/[id]` detail surface with loading, error, not-found, retry, educational disclaimer, public Product DTO display, and Save product / Remove from saved action. `components/admin-product-review.tsx` is the client-side `/admin/products` review surface with search/status filter, all-status list display, status update workflow, and loading/empty/error/unauthorized states. It does not implement Product submission, `POST /api/products`, `includeMine`, external product APIs, seed scripts, full Product CRUD UI, marketplace/cart/payment, comparison, recommendation, or image upload.

Current implemented saved-products files:
...
`saved-product.schema.ts` validates strict save bodies and route params with MongoDB ObjectId strings. `saved-product.repository.ts` imports `server-only`, uses `getSavedProductsCollection()`, scopes all queries by `userId`, upserts by `userId + productId`, handles duplicate-key races, and deletes only saved-product records. `saved-product.use-case.ts` confirms product existence through the Product use case before saving and maps saved records plus visible Product data to `SavedProductDto`. `saved-product.client.ts` is client-safe, fetches only `/api/saved-products`, validates API envelopes and DTO shapes, and preserves HTTP status/API error code where possible. Client components must not import the saved-products repository, use case, database helpers, auth helpers, API route handlers, or server-only modules.
...
Playwright smoke specs exist under `tests/e2e/smoke.spec.ts`. Current E2E coverage is smoke-level only: the public landing page and unauthenticated protected-route redirects are covered, while authenticated E2E flows and real Google OAuth login are not tested in CI.
