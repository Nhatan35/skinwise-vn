...
`product.schema.ts` owns strict list query validation for `GET /api/products`. `product.repository.ts` imports `server-only`, uses `getProductsCollection()`, applies default visibility for `reviewed` and `verified` products only, supports canonical search/filter params, and returns `null` for invalid ObjectId detail lookups without querying. `product.mapper.ts` converts `_id` to `id`, Dates to ISO strings, copies arrays, and omits `createdByUserId`, `source`, and raw ObjectId values. `product.client.ts` is client-safe and fetches `GET /api/products` with supported search/filter params and default `limit=50`, parsing visible products from `data.items` for UI consumers; it also fetches `GET /api/products/[id]` through `getProduct()` and parses detail responses from `data.product`. `components/product-catalogue.tsx` is the client-side `/products` catalogue surface with search, category, price range, skin type, and concern filters; loading/error/empty states; and Save/Saved state from the Saved Products client. `components/product-card.tsx` displays public Product DTO fields, links to product detail pages, and renders the saved-product toggle. `components/product-detail.tsx` is the client-side `/products/[id]` detail surface with loading, error, not-found, retry, educational disclaimer, public Product DTO display, and Save product / Remove from saved action. It does not implement Product submission, `POST /api/products`, `includeMine`, admin product management, external product APIs, seed scripts, Product CRUD UI, marketplace/cart/payment, comparison, recommendation, or image upload.
