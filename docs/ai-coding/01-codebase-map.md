...
`auth.config.ts` is edge-safe and owns pure provider/config helpers. `src/auth.ts` owns the full server-side Auth.js setup with MongoDB Adapter gating. `src/proxy.ts` owns the Next.js 16 proxy wrapper for `/dashboard/:path*`, `/onboarding/:path*`, `/skin-profile/:path*`, `/routines/:path*`, `/journal/:path*`, and `/products/:path*`. `get-current-user.ts` maps Auth.js sessions to a minimal `CurrentUser` without querying `AppUserProfile`.
...
`routes.DASHBOARD` points to `/dashboard`, `routes.SKIN_PROFILE` points to `/skin-profile`, `routes.ONBOARDING_SKIN_PROFILE` points to `/onboarding/skin-profile`, `routes.ROUTINES` points to `/routines`, `routes.JOURNAL` points to `/journal`, and `routes.PRODUCTS` points to `/products`.
