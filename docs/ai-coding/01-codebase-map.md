...
`product.schema.ts` owns strict list query validation for `GET /api/products`. `product.repository.ts` imports `server-only`, uses `getProductsCollection()`, applies default visibility for `reviewed` and `verified` products only, supports canonical search/filter params, and returns `null` for invalid ObjectId detail lookups without querying. `product.mapper.ts` converts `_id` to `id`, Dates to ISO strings, copies arrays, and omits `createdByUserId`, `source`, and raw ObjectId values. `product.client.ts` is client-safe and fetches `GET /api/products` with supported search/filter params and default `limit=50`, parsing visible products from `data.items` for UI consumers. `components/product-catalogue.tsx` is the client-side `/products` catalogue surface with search, category, price range, skin type, and concern filters; loading/error/empty states; and no Product CRUD. `components/product-card.tsx` displays public Product DTO fields only. This foundation is read-only and is consumed by the Routine Builder Product Picker, server-side Routine Product Snapshot population, SkinJournal UI product name resolution, and Product Catalogue UI. It does not implement Product submission, `POST /api/products`, `includeMine`, admin product management, Product detail UI routes, external product APIs, seed scripts, saved product library, Product CRUD UI, or image upload.
