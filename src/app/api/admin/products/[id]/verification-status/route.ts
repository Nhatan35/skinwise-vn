...
import { updateProductVerificationStatusForAdmin } from "@/modules/products/product.use-case";
...
export async function PATCH(request: Request, context: RouteContext) {
  try {
    ...
    const input = updateProductVerificationStatusBodySchema.parse(
      await getRequestJson(request),
    );
    const product = await updateProductVerificationStatusForAdmin(
      id,
