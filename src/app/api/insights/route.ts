...
import { insightsQuerySchema } from "@/modules/insights/insights.schema";
...
export async function GET(request: Request) {
  try {
    ...
    const input = insightsQuerySchema.parse(getQueryParams(request));
    const insights = await getInsightsForUser(currentUser.id, input);

    return jsonResponse<{ insights: InsightsDto }>({ insights });
  } catch (error) {
