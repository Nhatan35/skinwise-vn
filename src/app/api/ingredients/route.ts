...
import { listIngredients } from "@/modules/ingredients/ingredient.use-case";
...
export async function GET(request: Request) {
  try {
    ...
    const input = ingredientListQuerySchema.parse(getQueryParams(request));
    const ingredients = await listIngredients(input);
