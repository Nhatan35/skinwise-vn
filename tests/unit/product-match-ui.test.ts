...
describe("Product Match UI source", () => {
  ...
  it("renders match cards with score, level, explanation, save, and detail actions", () => {
    for (const expectedSource of [
      ...
      "Vì sao sản phẩm này xuất hiện?",
      "Bước tiếp theo",
      "Xem chi tiết sản phẩm",
      "Mức độ phù hợp",
      "tín hiệu tương thích MVP",
      ...
      "usageNote",
      "matchedSignals",
      "Khớp với loại da đã lưu",
      "Liên quan đến mối quan tâm đã lưu",
      "Nằm trong khoảng ngân sách đã lưu",
      "Cần xem kỹ vì hồ sơ có độ nhạy cảm hoặc cảnh báo sản phẩm",
      "Có thành phần trùng với danh sách muốn tránh",
